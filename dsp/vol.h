...
// Runtime tuning knob: additional attenuation offset (in 1 dB steps) applied on top
// of the host-provided USB volume. Default is set in main.c.
extern int16_t g_volume_offset_steps;
...
static void audio_set_volume(int16_t v, int8_t ch)
{
    ...
    int32_t pos = (-v / 256) + (int32_t)g_volume_offset_steps; // headroom offset to avoid clipping and keep volume in safe range
